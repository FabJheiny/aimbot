...
local function AddBottomLeftResize(Window, Minimum, Maximum)
	...
	Connect(Services.Input.InputChanged, function(Input)
		...
		-- BottomLeft: dragging left → wider (negative X delta), dragging down → taller
		...
		-- Compensate position so the right edge stays fixed
...
local function CreateFloatingButton(Screen, OnClick)
	...
	do
		...
		Connect(Btn.InputBegan, function(Input)
			-- Only start drag after a short hold to distinguish click vs drag
			if Input.UserInputType == Enum.UserInputType.MouseButton1 then
				local t0 = tick()
